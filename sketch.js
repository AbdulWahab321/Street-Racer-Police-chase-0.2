...
async function preload() {
  ...
  PImg = loadImage("policeLight.png")
...
async function setup() {
  ...
  text("Loading Musics",50,100)
  song = loadSound("MP3/sonicastronomia.mp3")
  song.onloadstart=()=>{
    alert("Loading Music 1")
  }
  song.onloadeddata=()=>{
    text("Loaded Music 1",50,130)
  }
  spec = loadSound("MP3/spectre.mp3")
  text("Loaded Music 2",50,160)
  alo = loadSound("MP3/alone.mp3")
  text("Loaded Music 3",50,190)
  dar = loadSound("MP3/dark.mp3")
  text("Loaded Music 4",50,220)
  overSnd = loadSound("MAIN/over.wav")
  text("Loaded soundEffect",50,250)
  fad = loadSound("MP3/fade.mp3")
  text("Loaded Music 5",50,280)
  onmy = loadSound("MP3/onMyWay.mp3")
  text("Loaded Music 6",50,310)
  Her=loadSound("MP3/Her.mp3")
  text("Loaded Music 7",50,340)
  SpeNcs=loadSound("ncs.mp3")
  text("Loaded Music 8",50,370)

  conBack = createButton("Go Back to system confirmer")
  conBack.position(1, 479);

PauseButton=createButton("Pause")
PauseButton.position(900,10)
PauseButton.size(100,50)
PauseButton.hide();

RButton=createButton("Resume")
RButton.position(900,10)
RButton.size(100,50)
RButton.hide();
  ...
  MusSlider=createSlider(0, 360, 60, 40);
  MusSlider.position(10,30);
  MusSlider.hide();
...
async function draw() {
  ...
  window.onloadstart=()=>{
    alert("Loading Images and Musics")
  }

  window.onbeforeunload=()=>{
   def=confirm("Some Problem is there on loading Do You Want to Reload")
   if(def===true){
     location.reload();
   }
  }
...
if (frameCount - 1 === 0) {
  ...
  textSize(20)
  if (gameState === END) {
    text("Press Space to restart",150,450)
...
if(gameState!==FIRSTBG&&gameState!==PauseMode&&gameState!==START&&gameState!==SELECTOR&&gameState!==CARSELECTOR&&gameState!==SETTINGS&&gameState!==INS){
  stroke("red")
  strokeWeight(2)
...
say("Hai")
  ...
  if (gameState === START) {
    ...
    if (keyDown("enter")) {
      ...
      if (ino.indexOf("Name") !=-1 || ino == "" || ino == " ") {
        fill("red")
        textSize(40)
        text("Invalid Name", 380, 20)
  ...
  if (gameState === SELECTOR) {
    ...
    SPectreNcs=createSprite(400,200,400,50)
  ...
  buttun.hide();
    ...
    text("(Spectre-[NCS Release])",SPectreNcs.x-150,SPectreNcs.y)
    ...
    if (frameCount % 1 === 0) {
      if (mousePressedOver(SPectreNcs)) {
...
window.onload = () => {
  window.alert("Loaded Scripts. Loading Images and Musics This will take a Minute. Please Be Patient")
}
window.onclose = () => {
...
async function spawnPolice() {
  ...
  policeC.debug=true
  policeC.setCollider("rectangle",20,0,policeC.width-410,policeC.height-110)
